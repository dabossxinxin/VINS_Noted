...
PoseGraph::PoseGraph() {
	...
	t_optimization = std::thread(&PoseGraph::optimize4DoF, this);	// 默认开启4DoF优化线程
        ...
        if (cur_kf->has_loop) {
            ...
            if(cur_kf->sequence > 0) {
				posegraph_visualization->add_loopedge(P0, connected_P +
					Eigen::Vector3d(VISUALIZATION_SHIFT_X, VISUALIZATION_SHIFT_Y, 0));
            }
        }
    }
...
int PoseGraph::detectLoop(
    ...
    if (find_loop && frame_index > 50) {
        ...
        for (unsigned int i = 0; i < ret.size(); i++) {
			if (min_index == -1 ||
				(ret[i].Id < min_index && ret[i].Score > 0.015)) {
				min_index = ret[i].Id;
			}
        }
        return min_index;
    ...
    while(true) {
        ...
        if (cur_index != -1) {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
                ...
                Eigen::Vector3d tmp_t = Vector3d(t_array[i][0], t_array[i][1], t_array[i][2]);
...
void PoseGraph::savePoseGraph() {
	...
	pFile = std::fopen(file_path.c_str(), "w");
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
		...
		FILE *keypoints_file = std::fopen(keypoints_path.c_str(), "w");
...
void PoseGraph::loadPoseGraph() {
	...
	pFile = std::fopen(file_path.c_str(), "r");
    if (pFile == NULL) {
	...
	while (fscanf(pFile, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d", 
		...
		FILE *keypoints_file = NULL;
		keypoints_file = fopen(keypoints_path.c_str(), "r");
		st::vector<cv::KeyPoint> keypoints;
