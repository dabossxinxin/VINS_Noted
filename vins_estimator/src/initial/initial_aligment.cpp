...
void RefineGravity(
    ...
    for(int k = 0; k < 4; ++k) {
        ...
        for (frame_i = all_image_frame.begin(); 
			...
			tmp_A.block<3, 2>(0, 6) = frame_i->second.R.transpose() * dt * dt * 0.5 * Matrix3d::Identity() * lxly;
            tmp_A.block<3, 1>(0, 8) = frame_i->second.R.transpose() * (frame_j->second.T - frame_i->second.T) / 100.0;     
			tmp_b.block<3, 1>(0, 0) = frame_j->second.pre_integration->delta_p + frame_i->second.R.transpose() * frame_j->second.R * TIC[0] - TIC[0] - frame_i->second.R.transpose() * dt * dt *0.5 * g0;
